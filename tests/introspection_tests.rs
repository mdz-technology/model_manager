...
fn test_has_property_basic() {
        ...
        user.set("active", Value::from_bool(true)).unwrap();
...
fn test_get_property_type_basic() {
        ...
        data.set("string_prop", Value::from_str("test")).unwrap();
        data.set("number_prop", Value::from_number(123.45).unwrap()).unwrap();
        data.set("bool_prop", Value::from_bool(false)).unwrap();
        data.set("object_prop", Value::new_object()).unwrap();
        data.set("array_prop", Value::new_array()).unwrap();
...
fn test_get_property_names_basic() {
        ...
        employee.set("name", Value::from_str("Carlos Mendoza")).unwrap();
        employee.set("department", Value::from_str("Engineering")).unwrap();
        employee.set("salary", Value::from_number(75000.0).unwrap()).unwrap();
        employee.set("remote", Value::from_bool(true)).unwrap();
        employee.set("start_date", Value::from_str("2023-01-15")).unwrap();
...
fn test_count_properties_basic() {
        ...
        small_object.set("prop1", Value::from_str("value1")).unwrap();
        small_object.set("prop2", Value::from_str("value2")).unwrap();
...
fn test_introspection_with_nested_objects() {
        ...
        root.set("id", Value::from_number(1.0).unwrap()).unwrap();
        root.set("name", Value::from_str("Root Object")).unwrap();

        let mut nested = Value::new_object();
        nested.set("nested_prop", Value::from_str("nested_value")).unwrap();
        nested.set("nested_number", Value::from_number(99.9).unwrap()).unwrap();
        root.set("nested", nested).unwrap();

        let mut array = Value::new_array();
        array.push(Value::from_str("item1")).unwrap();
        array.push(Value::from_str("item2")).unwrap();
...
fn test_introspection_with_model_manager() {
        ...
        company.set("founded", Value::from_number(2020.0).unwrap()).unwrap();
        company.set("active", Value::from_bool(true)).unwrap();

        let mut address = Value::new_object();
        address.set("street", Value::from_str("123 Tech Street")).unwrap();
        address.set("city", Value::from_str("San Francisco")).unwrap();
        address.set("country", Value::from_str("USA")).unwrap();
...
fn test_introspection_comprehensive_scenario() {
        ...
        let mut employee = DefaultValue::create();
        employee.set("id", Value::from_number(12345.0).unwrap()).unwrap();
        employee.set("name", Value::from_str("María González")).unwrap();
        employee.set("email", Value::from_str("maria@techcorp.com")).unwrap();
        employee.set("active", Value::from_bool(true)).unwrap();
        ...
        profile.set("start_date", Value::from_str("2022-03-15")).unwrap();
        employee.set("profile", profile).unwrap();

        let mut skills = Value::new_array();
        skills.push(Value::from_str("Rust")).unwrap();
        skills.push(Value::from_str("JavaScript")).unwrap();
        skills.push(Value::from_str("Database Design")).unwrap();
        ...
        for i in 1..=3 {
            ...
            project.set("status", Value::from_str("active")).unwrap();
        ...
        assert_eq!(employee.get_property_type("id").unwrap(), Some("Number".to_string()));
        assert_eq!(employee.get_property_type("name").unwrap(), Some("String".to_string()));
        assert_eq!(employee.get_property_type("email").unwrap(), Some("String".to_string()));
        assert_eq!(employee.get_property_type("active").unwrap(), Some("Bool".to_string()));
        assert_eq!(employee.get_property_type("profile").unwrap(), Some("Object".to_string()));
        assert_eq!(employee.get_property_type("skills").unwrap(), Some("Array".to_string()));
        assert_eq!(employee.get_property_type("projects").unwrap(), Some("Array".to_string()));
