...
fn test_json_converter_basic() {
        ...
        assert_eq!(dynamic_value.get("name").unwrap().unwrap().as_str().unwrap(), "Test User");
        assert_eq!(dynamic_value.get("age").unwrap().unwrap().as_number().unwrap(), 30.0);
        assert_eq!(dynamic_value.get("active").unwrap().unwrap().as_bool().unwrap(), true);
...
fn test_json_converter_complex_data() {
        ...
        assert_eq!(user.get("name").unwrap().unwrap().as_str().unwrap(), "Ana García");

        let profile = user.get("profile").unwrap().unwrap();
        assert_eq!(profile.get("department").unwrap().unwrap().as_str().unwrap(), "Engineering");
        assert_eq!(profile.get("level").unwrap().unwrap().as_number().unwrap(), 5.0);

        let items = dynamic_value.get("items").unwrap().unwrap();
        let items_array = items.as_array().unwrap().unwrap();
        assert_eq!(items_array.len(), 2);
        assert_eq!(items_array[0].get("id").unwrap().unwrap().as_number().unwrap(), 1.0);
...
fn test_json_converter_batch_operations() {
        ...
        for (i, value) in dynamic_values.iter().enumerate() {
            assert_eq!(value.get("id").unwrap().unwrap().as_number().unwrap(), (i + 1) as f64);
            assert_eq!(value.get("name").unwrap().unwrap().as_str().unwrap(), format!("User {}", i + 1));
        }
...
fn test_json_converter_pretty_printing() {
        ...
        data.set("name", Value::from_str("Pretty Test")).unwrap();
        data.set("age", Value::from_number(25.0).unwrap()).unwrap();

        let mut nested = Value::new_object();
        nested.set("department", Value::from_str("Engineering")).unwrap();
...
fn test_json_converter_with_custom_config() {
        ...
        let mut data = DefaultValue::create();
        data.set("test", Value::from_str("custom config")).unwrap();
