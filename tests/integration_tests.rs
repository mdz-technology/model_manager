...
async fn test_basic_crud_operations() {
    ...
    local_set.run_until(async {
        ...
        user_data.set("name", Value::from_str("John Doe")).await.unwrap();
        user_data.set("age", Value::from_number(30.0).unwrap()).await.unwrap();
        ...
        updated_data.set("name", Value::from_str("Jane Doe")).await.unwrap();
        updated_data.set("age", Value::from_number(25.0).unwrap()).await.unwrap();
...
async fn test_multiple_models() {
    ...
    local_set.run_until(async {
        ...
        user.set("name", Value::from_str("Test User")).await.unwrap();

        let mut product = DefaultValue::create();
        product.set("name", Value::from_str("Test Product")).await.unwrap();
        product.set("price", Value::from_number(99.99).unwrap()).await.unwrap();
...
async fn test_actor_system_stress() {
    ...
    local_set.run_until(async {
        ...
        for i in 0..50 {
            ...
            data.set("id", Value::from_number(i as f64).unwrap()).await.unwrap();
            data.set("name", Value::from_str(&format!("User {}", i))).await.unwrap();
            data.set("batch", Value::from_str("stress_test")).await.unwrap();
        ...
        for i in 0..10 {
            ...
            let mut product = DefaultValue::create();
            product.set("name", Value::from_str(&format!("Product {}", i))).await.unwrap();
            product.set("price", Value::from_number(10.0 * i as f64).unwrap()).await.unwrap();
...
async fn test_dynamic_value_operations() {
    ...
    local_set.run_until(async {
        ...
        obj.set("string_field", Value::from_str("test")).await.unwrap();
        obj.set("number_field", Value::from_number(42.5).unwrap()).await.unwrap();
        obj.set("bool_field", Value::from_bool(true)).await.unwrap();
        ...
        arr.push(Value::from_str("item1")).await.unwrap();
        arr.push(Value::from_number(123.0).unwrap()).await.unwrap();
